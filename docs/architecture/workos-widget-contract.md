...
- JWT template that sets `aud` only when using a custom `WORKOS_TOKEN_AUDIENCE`; default AuthKit session tokens omit `aud` and bind via JWKS/`client_id` (missing `client_id` allowed after signature verify)
