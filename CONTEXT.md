...
**Owner**:
The single Member who administers a Household: inviting, removing Members, renaming, transferring ownership, or deleting the Household. An Owner cannot leave their own Household — they must transfer ownership first or delete it. Later WorkOS admin/member/viewer roles supersede this unique Owner model.
_Avoid_: Admin, Creator

**Invite Code**:
A short, expiring, optionally single-use code generated by an Owner that admits one User into a Household as a Member. Codes are shared out-of-band; there is no email delivery. Expired, revoked, or consumed codes are rejected without side effects. WorkOS invitations supersede Invite Codes in later Household administration work.
