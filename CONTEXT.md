...
**Membership**:
The association of one User with one Household carrying a role and an active flag. A User holds at most one active Membership at a time; the active one is the Household the app currently addresses. Memberships support multiple Households per User even though only one is active.
_Avoid_: Account Link, Subscription

**Owner**:
The single Member who administers a Household: inviting, removing Members, renaming, transferring ownership, or deleting the Household. An Owner cannot leave their own Household — they must transfer ownership first or delete it.
_Avoid_: Admin, Creator

**Invite Code**:
A short, expiring, optionally single-use code generated by an Owner that admits one User into a Household as a Member. Codes are shared out-of-band; there is no email delivery. Expired, revoked, or consumed codes are rejected without side effects.
