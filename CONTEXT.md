...
**Household**:
A shared money-management workspace backed by one WorkOS Organization and one organization Ledger. Multiple Users may join it through WorkOS invitations; every Household Account and its financial activity is visible to all active Members. Creating or joining starts a separate shared Ledger and never moves or exposes Personal Ledger data.
...
**Ledger**:
The row that makes a Ledger Scope addressable, identified by a Ledger id: `personal:<workos-user-id>` for personal scope and the WorkOS Organization id for organization scope. Every Account, Category, and Transaction belongs to exactly one Ledger, and nothing crosses between two — an Account and the Category it is booked against must share a Ledger. A personal Ledger exists from its User's first write; it has no Members or invitations.
_Avoid_: Book, Tenant, Workspace

**Membership**:
The WorkOS-owned association of one User with one Household, carrying an `admin`, `member`, or `viewer` role and an active, pending, or inactive status. Trove stores only a derived projection maintained from verified events and authoritative bootstrap reads; unknown roles and non-active statuses deny access. A User may hold active Memberships in multiple Households, while device-local Ledger Selection independently chooses which Ledger the app currently addresses.
_Avoid_: Account Link, Subscription

**Household Admin**:
An active Member with WorkOS's `admin` role. Admins may invite, remove, and change Member roles through the WorkOS management surface and may rename or delete the Household through Trove. Trove has no separate Owner role or ownership-transfer authority.
_Avoid_: Owner, Creator

**WorkOS Invitation**:
An email invitation issued by a Household Admin through WorkOS for a specific Organization and role. Acceptance may open WorkOS-hosted invitation handling and AuthKit, but it is distinct from identity sign-in and never carries a Trove Invite Code.
_Avoid_: Invite Code, Sign-in Magic Link, Password

> **Naming note**: "Account" in this context always means a _financial_ Account (see Cash Account, Archived Account). Legacy Better Auth tables may still exist in the database until the development reset; they are plumbing and carry no domain meaning.

**Command**:
The unit of a client write to the backend: a domain intent (e.g. `account.update`, `transaction.create`) — not a row diff — carrying a client-generated `commandId` that doubles as its idempotency key. Applied optimistically to the PowerSync collection, stored with its full envelope in the PowerSync upload queue, and processed exactly once server-side regardless of retry count. Membership mutations are not Commands; WorkOS owns them. Wire shape lives in `packages/protocol`.
