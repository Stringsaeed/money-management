...
describe("household-scoped projection and drain", () => {
  ...
  it("rebases a later Account restore after an invalid archive is rejected", async () => {
    const db = await setupDb();
    await enqueueCommand(
      db,
      makeInput({
        commandId: "cmd-archive-invalid",
        userId: "user-1",
        kind: "account.archive",
        payload: { accountId: "account-1" },
        preconditions: [{ entityId: "account-1", expectedVersion: 4 }],
    ...
    const summary = await drainOutbox(
      ...
      async (envelope) => {
        sent.push(envelope);
        return sent.length === 1
          ? { kind: "invalid_intent", issues: [{ field: "accountId", message: "funding" }] }
          : applied();
      },
      "user-1",
    );
