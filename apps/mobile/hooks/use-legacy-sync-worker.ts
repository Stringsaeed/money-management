...
interface LegacyLedgerObserver {
  readonly settle: (
    envelope: CommandEnvelope,
    result: Awaited<ReturnType<typeof orpc.commands.apply>>,
  ) => void;
  readonly noteRemoteChanges: (
 ...
 */
export function useLegacySyncWorker(householdId: string | null, userId?: string) {
  ...
  const ledgerRef = useRef<LegacyLedgerObserver | null>(null);
...
interface SyncTurnInput {
  ...
  readonly ledger: LegacyLedgerObserver | null;
...
const applyDrainEffects = async (
  ...
  ledger: LegacyLedgerObserver | null,
): Promise<void> => {
...
const applyAndSettle = async (envelope: CommandEnvelope, ledger: LegacyLedgerObserver | null) => {
