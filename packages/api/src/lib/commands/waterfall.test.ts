...
describe("interleaving safety — preconditions replace locks", () => {
  ...
  it("aborts the second stale batch before it double-spends Unassigned Money", async () => {
    ...
    await executeBatch(db, buildStatements(crypto.randomUUID(), planA), HOUSEHOLD_ID);
    await expect(
      executeBatch(db, buildStatements(crypto.randomUUID(), planB), HOUSEHOLD_ID),
    ).rejects.toThrow();
  ...
  it("converges under randomized interleavings — Unassigned Money never goes negative (fuzz)", async () => {
    ...
    for (let round = 0; round < 30; round += 1) {
      ...
      try {
        await executeBatch(db, buildStatements(crypto.randomUUID(), outcome), HOUSEHOLD_ID);
        committed += 1;
      } catch {
