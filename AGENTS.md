...
- Follow everything here before touching CLAUDE.md, because this file already merges those instructions plus extra context.
...
- Start by skimming CLAUDE.md and this file to refresh requirements for linting, formatting, skills, and Expo architecture.
...
- After editing, rerun `bun run lint:fix && bun run format` even if linters previously passed—this is mandatory per CLAUDE.md.
...
- `global.css`, `postcss.config.mjs`, and `nativewind-env.d.ts` set up Tailwind v4 via `react-native-css` and NativeWind.
...
- Use `clsx` or `tailwind-merge` already installed when combining class strings for NativeWind.
...
- Before writing Expo or React Native code, load relevant skills listed in CLAUDE.md (e.g., `building-ui`, `data-fetching`, `react-native-best-practices`).
...
- [ ] Read this file and CLAUDE.md before coding.
