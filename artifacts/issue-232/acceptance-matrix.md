...
| HEAD                | tip after this Relates stamp — **DDL cred HARD STOP + PlanetScale schema reconfirm PASS** (0011–0015 already on `trove/main`; Deploy ensure `42501`→`42703` is deploy-role ALTER denial, not missing privileged schema); remaining **BLOCKED/FAIL**: webhook **503** (awaits owner merge #266 + Deploy), #258–#265 Create Account/NativeButton **PARKED**, dual-token/`CERT_USER_*`, GH Actions billing, Android, OTP AX, PowerSync mint, signed-in Mac session |
...
| Worktree            | `/tmp/wt-241-cert232-ddl` (this stamp); prior `/tmp/wt-241/money-management` local CI; Mac evidence from Muhammed's Mac self-hosted worker `1243a933-…`                                                                                                                                                                                   |
| Live API            | `https://auth.trove.ing` — root **200 OK**; Sync `getManifest` CF Worker **200** post-DDL; webhook still **503** after #259 (Deploy [34713619532](https://github.com/Stringsaeed/money-management/actions/runs/34713619532) schema-ensure `42501`→`42703`; secret never reached prod). Schema on PlanetScale still **PASS** — see `ddl-cred-probe-and-schema-reconfirm-2026-09-12T1948Z.md`. |
