...
| HEAD                | `PRODUCT_TIP` — `accountLifecyclePeriod` **2/2** jest; `mapSessionSnapshot` **4/4** jest; `isUnreachableFailure` **2/2** jest; `householdsQueryKeyForUser` / `householdUserIdForQuery` **2/2** jest; prior `3a45133` / `38a5944` access+API titles; remaining **BLOCKED**: webhook **503**, #258 Create Account, dual-token, GH Actions billing, Android, OTP AX, PowerSync mint  Provenance          | Squash merge of #240 / closes #231 on `main`, plus [#242](https://github.com/Stringsaeed/money-management/pull/242), [#245](https://github.com/Stringsaeed/money-management/pull/245), [#246](https://github.com/Stringsaeed/money-management/pull/246), and schema **0011–0015** on PlanetScale `trove/main`.   |
| Worktree            | `/tmp/wt232` (cloud accountLifecyclePeriod + session snapshot/unreachable + household query-key titles); prior Mac evidence from `/Users/saeed/Work/money-management-wt-232`                                                                                                                                             |
