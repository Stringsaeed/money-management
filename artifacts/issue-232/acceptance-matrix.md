...
| HEAD                | `PRODUCT_SHA` — `describeEffectTag` / `summarizeEffects` **6/6** vitest; `issuesFromZod` / `checkExpectedVersion` / `scopeColumns` **4/4** vitest; `isKillSwitchEngaged` **2/2** vitest; `sha256Hex` **1/1** vitest; `encodeLedgerSelection` / `decodeLedgerSelection` **3/3** jest; `personalImportBinding` **1/1** jest; `serializeReturnTo` / `returnTo` **4/4** jest; `canPresentAuthSheet` / `openAuthSheetSession` **6/6** jest; prior `59fe9c7` native-ui/identity/claim; remaining **BLOCKED**: webhook **503**, #258 Create Account, dual-token, GH Actions billing, Android, OTP AX, PowerSync mint |
...
| Worktree            | `/tmp/wt232` (cloud API effect/command/kill-switch + access encode/import/return/sheet titles); prior Mac evidence from `/Users/saeed/Work/money-management-wt-232`                                                                                                                                             |
