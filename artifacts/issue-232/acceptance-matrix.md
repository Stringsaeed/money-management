...
| HEAD                | tip `0633987` access/sql/widget-token stamp — product `b45082be6f39da2fc5896e71166645105bd89bcb` (`queryRows` **3/3**; `ledgerReadInput` **5/5**; `authFailureFromClient` **2/2**; `hrefForInternal` / `parseReturnTo` **5/5**; `decideWidgetToken` **1/1**; `readBearerToken` / `jwksUrlForClient` **1/1**); remaining **BLOCKED**: webhook **503**, #258 Create Account, dual-token, GH Actions billing, Android, OTP AX, PowerSync mint |
