...
| HEAD                | `PRODUCT_TIP` — `toMembershipSummary` / `roleLabel` **4/4** jest; `nextClaim` **4/4** jest; `pickActiveHousehold` **2/2** jest; `resolveReturnDestination` **3/3** jest; `householdReadFromQuery` **4/4** jest; `commandEnvelopeSchema` **5/5** vitest; `rejectionReason` **1/1** vitest; prior `38a5944` effect/command/access codec; remaining **BLOCKED**: webhook **503**, #258 Create Account, dual-token, GH Actions billing, Android, OTP AX, PowerSync mint 
...
| Worktree            | `/tmp/wt232` (cloud access membership/claim/household + command envelope/rejectionReason titles); prior Mac evidence from `/Users/saeed/Work/money-management-wt-232`                                                                                                                                             |
