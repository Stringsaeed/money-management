...
| HEAD                | `a284a96` — `parseSetupAssignment` **10/10** jest; `evaluateCardBudgetState` **4/4** jest; `initialPowerSyncAvailability` / `recordPowerSyncConnection` / `isPowerSyncUnavailable` **2/2** jest; prior `51fc6d5` lifecycle/session/query-key; remaining **BLOCKED**: webhook **503**, #258 Create Account, dual-token, GH Actions billing, Android, OTP AX, PowerSync mint |
