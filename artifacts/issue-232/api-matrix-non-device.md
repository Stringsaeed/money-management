...
| WorkOS webhook / membership projection | **BLOCKED** (prod) | `POST /webhooks/workos` → **503** empty `WORKOS_WEBHOOK_SECRET` (`workos-webhook-blocked.md`) | Re-probe 05:47Z still **503** (`workos-webhook-probe-2026-09-12b.txt`) |
...
| 4 Households | **PARTIAL** + webhook **BLOCKED** | API seams only; live create/switch/invite not evidenced; prod webhook **503** (re-probe 05:47Z). |
