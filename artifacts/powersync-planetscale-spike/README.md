...
This directory is the Z0 proof. It does not change `apps/`, `packages/`, or `tools/`. The operator database is PlanetScale Postgres `trove` in org `stringsaeed`. Workers use a cache-disabled Hyperdrive named `trove-ledger-fresh`. PowerSync Cloud replicates over the direct `:5432` host as the replication role, never through Hyperdrive.
