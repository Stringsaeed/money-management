...
- `jq`, `python3`, `curl`, `npm`
- PowerSync Cloud credentials in the environment. Use `PS_ADMIN_TOKEN`, or set `POWERSYNC_URL` and `POWERSYNC_TOKEN` after you mint a development token

Do not put passwords in this directory. `smoke.sh` writes them to `/tmp/z0-spike.env` with mode `600`.
...
4. `CREATE PUBLICATION ... FOR ALL TABLES` is rejected.
